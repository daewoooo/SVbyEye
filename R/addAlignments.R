...
#'addAlignments(ggplot.obj = plt, paf.table = paf.gaps,
...
addAlignments <- function(ggplot.obj=NULL, paf.table=NULL, color.by=NULL, color.palette=NULL, fill.by=NULL, fill.palette=NULL, outline.alignments=FALSE, linetype='solid', coord.strict=TRUE) {
  ...
  if (!is.null(linetype)) {
    ...
    if (!linetype %in% allowed.linetypes) {
      w.msg <- c("Parameter linetype can only take values [",
  ...
  if (!is.null(color.by)) {
    ggplot.obj <- ggplot.obj + ggnewscale::new_scale_color() +
      geom_miropeats(data = coords, ggplot2::aes(x = .data$x, y = .data$y, group = .data$group, color = .data[[color.by]]),
    ...
    ggplot.obj <- ggplot.obj + ggnewscale::new_scale_fill() +
      geom_miropeats(data = coords, ggplot2::aes(x = .data$x, y = .data$y, group = .data$group, fill = .data[[fill.by]]),
    ...
    ggplot.obj <- ggplot.obj +
      geom_miropeats(data=coords, ggplot2::aes(x = .data$x, y = .data$y, group = .data$group),
