...
paf2gaps <- function(paf.table, min.gap.diff=NULL) {
  ...
  for (i in seq_along(paf.l)) {
    ...
    t.ranges <- GenomicRanges::GRanges(seqnames = sub.paf$t.name,
                                       ranges = IRanges::IRanges(start = sub.paf$t.start, end = sub.paf$t.end))
    q.ranges <- GenomicRanges::GRanges(seqnames = sub.paf$q.name,
    ...
    paf.gaps <- tibble::tibble(q.name = unique(sub.paf$q.name),
...
#'target.ranges <- GenomicRanges::GRanges(seqnames = 'target',
#'                                        IRanges::IRanges(start = c(1, 111, 150),
#'                                                         end = c(100, 150, 200)))
#'query.ranges <- GenomicRanges::GRanges(seqnames = 'query',
#'                                       IRanges::IRanges(start = c(1000, 1100, 1301),
  ...
  if (length(gr) > 1) {
    ...
    if (length(gap.gr) > 0) {
      ...
      gap.gr$up.gr <- GenomicRanges::GRanges(seqnames = GenomeInfoDb::seqnames(gap.gr),
                                             ranges = IRanges::IRanges(start = GenomicRanges::start(gap.gr),
      ...
      gap.gr$down.gr <- GenomicRanges::GRanges(seqnames = GenomeInfoDb::seqnames(gap.gr),
                                               ranges = IRanges::IRanges(start = GenomicRanges::end(gap.gr),
      ...
      ## Upstream
      gap.gr$aln.up.gr <- GenomicRanges::GRanges(seqnames = GenomeInfoDb::seqnames(gap.gr),
                                                 ranges = IRanges::IRanges(start = GenomicRanges::start(gap.gr),
                                                                           end = GenomicRanges::start(gap.gr)))
      suppressWarnings( gap.gr$aln.up.gr[up.idx.keep] <- gr$aln.gr[up.idx[up.idx.keep]][,0] )
      ## Downstream
      gap.gr$aln.down.gr <- GenomicRanges::GRanges(seqnames = GenomeInfoDb::seqnames(gap.gr),
                                                   ranges = IRanges::IRanges(start = GenomicRanges::end(gap.gr),
                                                                             end = GenomicRanges::end(gap.gr)))
      ...
      aln.gap.gr <- GenomicRanges::GRanges(seqnames = GenomeInfoDb::seqnames(gap.gr$aln.up.gr),
