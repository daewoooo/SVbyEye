...
Of course more complex self-alignments can be visualized as well in order to gain an overview of all pairs of segmental duplications that can facilitate  rearrangements between them.
...
plt + ggplot2::theme(panel.border = ggplot2::element_rect(colour = "black", 
                                                          fill=NA, linewidth=1),
                     legend.position = 'top')
```
...
The SVbyEye has its own default color scheme to color alignments by their direction as well as their sequence identity. However, these default color schemes can be changed either by setting your own color for alignment direction as explained in section [Visualization of sequence-to-sequence alignments](#Visualization of sequence-to-sequence alignments). In case of changing the color scheme for sequence identity a user will have to define an own discrete variable with corresponding color scheme in order to change the default color scheme. The process of how to do this is explained below.
